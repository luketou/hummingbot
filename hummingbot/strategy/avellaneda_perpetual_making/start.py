...
async def start(self):
    ...
    try:
        ...
        debug_csv_path = os.path.join(
            data_path(),
            HummingbotApplication.main_application().strategy_file_name.rsplit(".", 1)[
                0
            ]
        ...
        if isinstance(risk_factor, str) and risk_factor.lower() in [
            "adaptive",
            "simple_adaptive",
        ]:
            ...
            if risk_factor.lower() == "adaptive":
                ...
                adaptive_gamma_learning_rate = getattr(
                    c_map, "adaptive_gamma_learning_rate", 0.01
                )
                adaptive_gamma_min = getattr(c_map, "adaptive_gamma_min", 0.1)
                adaptive_gamma_max = getattr(c_map, "adaptive_gamma_max", 10.0)
                adaptive_gamma_reward_window = getattr(
                    c_map, "adaptive_gamma_reward_window", 100
                )
                adaptive_gamma_update_frequency = getattr(
                    c_map, "adaptive_gamma_update_frequency", 10
                )
            ...
            risk_factor_value = (
                risk_factor
                if hasattr(risk_factor, "quantize")
                else Decimal(str(risk_factor))
            )
        ...
        strategy.init_params(
            ...
            momentum_window_short=c_map.momentum_window_short,
            momentum_window_long=c_map.momentum_window_long,
            order_flow_window=c_map.order_flow_window,
            funding_rate_bias_enabled=c_map.funding_rate_bias_enabled,
            funding_rate_weight=c_map.funding_rate_weight,
            momentum_weight=c_map.momentum_weight,
            order_flow_weight=c_map.order_flow_weight,
            ...
            long_profit_taking_spread=c_map.long_profit_taking_spread
            ...
            exchange_preplaced_stop_loss=c_map.exchange_preplaced_stop_loss,
            stop_loss_working_type=c_map.stop_loss_working_type,
        ...
        # Success message
        self.notify(
            "✅ Avellaneda Perpetual Market Making strategy initialized successfully!"
        )
        ...
        self.logger().info(
            f"   🎯 Target Inventory: {c_map.inventory_target_base_pct}%"
        )
        ...
        if adaptive_gamma_enabled:
            ...
            self.logger().info(
                f"   📊 Gamma Range: [{adaptive_gamma_min}, {adaptive_gamma_max}]"
            )
        ...
        self.logger().info(
            f"   💚 Long Profit Taking: {c_map.long_profit_taking_spread}%"
        )
        self.logger().info(
            f"   💛 Short Profit Taking: {c_map.short_profit_taking_spread}%"
        )
        self.logger().info(f"   ❌ Stop Loss: {c_map.stop_loss_spread}%")
        self.logger().info(
            f"   🧷 Exchange Preplaced Stop Loss: {c_map.exchange_preplaced_stop_loss}"
        )
        self.logger().info(
            f"   🧭 Stop Loss Trigger Source: {c_map.stop_loss_working_type}"
        )
        ...
        self.notify(
            f"❌ Error initializing Avellaneda Perpetual Making strategy: {str(e)}"
        )
...
def validate_market_trading_pair(market_info):
    ...
    try:
        ...
        if quote not in valid_quotes:
            print(
                f"⚠️  Warning: {quote} is not a common quote currency for perpetual futures"
            )
    ...
    try:
        ...
        if derivative and derivative.value:
            ...
            if derivative.value not in derivative_connectors:
                errors.append(
                    f"Connector {derivative.value} does not support perpetual futures"
                )
        ...
        if market and market.value:
            try:
                ...
                if not base or not quote:
                    errors.append(
                        "Invalid trading pair format. Use BASE-QUOTE format (e.g., BTC-USDT)"
                    )
            except ValueError:
                errors.append(
                    "Invalid trading pair format. Use BASE-QUOTE format (e.g., BTC-USDT)"
            ...
            if isinstance(risk_factor.value, str):
                if risk_factor.value.lower() not in ["adaptive", "simple_adaptive"]:
                    try:
                        ...
                        errors.append(
                            "Risk factor must be a positive number or 'adaptive'"
                        )
        ...
        if all(
            p and p.value for p in [min_spread, long_profit, short_profit, stop_loss]
        ):
            if long_profit.value <= min_spread.value:
                errors.append(
                    "Long profit spread should be greater than minimum spread"
                )
            if short_profit.value <= min_spread.value:
                errors.append(
                    "Short profit spread should be greater than minimum spread"
                )
        ...
        if adaptive_enabled and adaptive_enabled.value:
            ...
            if all(g and g.value for g in [gamma_min, gamma_max, gamma_initial]):
                ...
                if not (gamma_min.value <= gamma_initial.value <= gamma_max.value):
                    errors.append(
                        "Initial gamma must be between minimum and maximum gamma"
                    )
