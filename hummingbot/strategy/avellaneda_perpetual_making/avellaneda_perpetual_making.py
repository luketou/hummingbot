...
from hummingbot.strategy.__utils__.trailing_indicators.trading_intensity import (
...
class AvellanedaPerpetualMakingStrategy(StrategyPyBase):
    ...
    def __init__(self):
        ...
        self._filled_order_delay = (
            15.0  # Default value, will be overridden in init_params
        )
    ...
    def init_params(
        ...
        self._avg_vol = InstantVolatilityIndicator(
            sampling_length=volatility_buffer_size
        )
        self._ticks_to_be_ready = max(
            volatility_buffer_size, trading_intensity_buffer_size
        )

        # Ensure minimum buffer sizes for stability
        if volatility_buffer_size < 50:
            self.logger().warning(
                f"⚠️  volatility_buffer_size ({volatility_buffer_size}) is too small for stable calculations. Recommended: ≥50"
            )
        if trading_intensity_buffer_size < 50:
            self.logger().warning(
                f"⚠️  trading_intensity_buffer_size ({trading_intensity_buffer_size}) is too small for stable calculations. Recommended: ≥50"
            )
        ...
        self.logger().info(
            "   🔧 ORDER MANAGEMENT FIX v2.0 - WITH CONFIRMATION MECHANISM"
        )
        ...
        self.logger().info(
            f"   🎯 Target Inventory: {self._inventory_target_base_pct}%"
        )
    ...
    def _initialize_adaptive_gamma(
        self,
        ...
        try:
            ...
            self.logger().info(
                f"🧠 Adaptive gamma learning enabled - "
    ...
    def calculate_reservation_price_and_optimal_spread(self):
        ...
        try:
            ...
            if positions:
                ...
                q = total_position / (
                    self._order_amount * 10
                )  # Scale by typical position size
            ...
            if self._alpha is None or self._kappa is None or self._kappa <= 0:
                ...
                if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                    self.logger().debug(
                        f"📊 Using default liquidity parameters: α={alpha}, κ={kappa}"
                    )
            else:
                alpha = self._alpha
                kappa = max(
                    self._kappa, Decimal("50.0")
                )  # Minimum kappa to prevent spread explosion

                if kappa != self._kappa:
                    self.logger().warning(
                        f"⚠️ Adjusted kappa from {self._kappa} to {kappa} to prevent spread explosion"
                    )
            ...
            if kappa > 0:
                try:
                    ...
                    if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                        ...
                        self.logger().debug(
                            f"   Liquidity term (2ln(1+γ/κ)/γ): {liquidity_term:.8f}"
                        )
                        self.logger().debug(
                            f"   Total spread: {self._optimal_spread:.8f}"
                        )
            ...
            if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                ...
                self.logger().debug(
                    f"   Calculated spread: {self._optimal_spread:.8f} ({calculated_spread_pct:.4f}%)"
                )
                self.logger().debug(
                    f"   Minimum spread: {min_spread_abs:.8f} ({min_spread_pct:.4f}%)"
                )
            ...
            if self._force_min_spread:
                if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                    ...
                    self.logger().info(
                        f"🚀 FORCE MIN SPREAD MODE - Volume farming activated"
                    )
                    self.logger().info(
                        f"   Avellaneda calculated: {self._optimal_spread:.8f} ({avellaneda_spread_pct:.4f}%)"
                    )
                    self.logger().info(
                        f"   Forcing to minimum: {min_spread_abs:.8f} ({min_spread_pct:.4f}%)"
                    )

                self._optimal_spread = min_spread_abs  # Always use minimum spread
            elif self._optimal_spread < min_spread_abs:
                self.logger().warning(
                    f"⚠️ Calculated spread {self._optimal_spread:.8f} below minimum {min_spread_abs:.8f}, applying minimum"
                )
            ...
            if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                ...
                self.logger().info(
                    f"   Risk Factor (γ): {gamma:.6f} {'(adaptive)' if self._use_adaptive_gamma else '(fixed)'}"
                )
                self.logger().info(
                    f"   Reservation Price: {self._reservation_price:.6f}"
                )
                self.logger().info(
                    f"   Optimal Spread: {self._optimal_spread:.6f} ({spread_pct:.2f}%)"
                )
                self.logger().info(
                    f"   Optimal Bid: {self._optimal_bid:.6f} ({((self._optimal_bid / current_price - 1) * 100):+.2f}%)"
                )
                self.logger().info(
                    f"   Optimal Ask: {self._optimal_ask:.6f} ({((self._optimal_ask / current_price - 1) * 100):+.2f}%)"
                )
    ...
    def update_adaptive_gamma(self):
        ...
        try:
            ...
            spread = float(
                self._optimal_spread / self.get_price()
                if self._optimal_spread > 0
                else Decimal("0.01")
            )
            ...
            if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                ...
                self.logger().debug(
                    f"   Inventory Deviation: {inventory_deviation:.6f}"
                )
    ...
    def create_base_proposal(self) -> Proposal:
        ...
        bid_price = market.quantize_order_price(
            self._market_info.trading_pair, self._optimal_bid
        )
        ask_price = market.quantize_order_price(
            self._market_info.trading_pair, self._optimal_ask
        )
        order_size = market.quantize_order_amount(
            self._market_info.trading_pair, self._order_amount
        )
    ...
    def _apply_adjusted_candidates_to_proposal(
        self, adjusted_candidates, proposal: Proposal
    ):
            ...
            if position.amount > 0:  # Long position
                if ask_price > position.entry_price:  # Profitable
                    profit_price = position.entry_price * (
                        Decimal("1") + self._long_profit_taking_spread
                    )
                    price = market.quantize_order_price(
                        self._market_info.trading_pair, profit_price
                    )
                    size = market.quantize_order_amount(
                        self._market_info.trading_pair, abs(position.amount)
                    )
                ...
                if bid_price < position.entry_price:  # Profitable
                    profit_price = position.entry_price * (
                        Decimal("1") - self._short_profit_taking_spread
                    )
                    price = market.quantize_order_price(
                        self._market_info.trading_pair, profit_price
                    )
        ...
        for position in positions:
            ...
            if position.amount > 0:  # Long position
                stop_loss_price = position.entry_price * (
                    Decimal("1") - self._stop_loss_spread
                )
    ...
    def _execute_orders_proposal(
        ...
        for buy in proposal.buys:
            order_id = self._market_info.market.buy(
                ...
                else None,  # Market orders don't need price
    ...
    def cancel_active_orders(self, proposal: Proposal = None):
        ...
        if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
            buy_orders = [
                o for o in exchange_active_orders if self._is_buy_order(o) is True
            ]
            sell_orders = [
                o for o in exchange_active_orders if self._is_buy_order(o) is False
            ]
            self.logger().debug(
                f"📋 Checking {len(buy_orders)} buy orders and {len(sell_orders)} sell orders for cancellation"
            )
        ...
        for order in exchange_active_orders:
            ...
            creation_timestamp = float(
                getattr(order, "creation_timestamp", self.current_timestamp)
            )
            ...
            effective_refresh_time = (
                self._order_refresh_time - 1.0
            )  # Cancel 1 second before refresh
            ...
            # 2. Cancel by price deviation to prevent stale quotes
                ...
                proposal is not None and self._optimal_bid > 0 and self._optimal_ask > 0
            ):
                order_price = getattr(order, "price", None)
                order_price_decimal = (
                    Decimal(str(order_price)) if order_price is not None else None
                )
                is_buy_order = self._is_buy_order(order)

                if (
                    order_price_decimal is None
                    or order_price_decimal <= 0
                    or is_buy_order is None
                ):
                    continue
                ...
                if is_buy_order:
                    ...
                    price_deviation_pct = (
                        abs(order_price_decimal - self._optimal_bid)
                        / self._optimal_bid
                        * 100
                    )
                    ...
                    price_deviation_pct = (
                        abs(order_price_decimal - self._optimal_ask)
                        / self._optimal_ask
                        * 100
                    )
                    if price_deviation_pct > float(self._order_refresh_tolerance_pct):
            ...
            if should_cancel:
                ...
                if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                    ...
                    self.logger().info(
                        f"🔄 Cancelling {side} order {order_id[:8]}... - Reason: {cancel_reason}"
                    )
        ...
        for order in orders_to_cancel:
            try:
                ...
                if order_id is None:
                    self.logger().warning(
                        f"⚠️ Skipping cancel for order without id: {order}"
                    )
                    continue
                self._market_info.market.cancel(
                    self._market_info.trading_pair, order_id
                )
                cancelled_count += 1
            except Exception as e:
                order_id = self._extract_order_id(order) or "unknown"
        ...
        if cancelled_count > 0:
            if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                self.logger().info(
                    f"📤 Cancelled {cancelled_count} orders, waiting for exchange confirmation..."
                )
    ...
    def start(self, clock: Clock, timestamp: float):
        """Strategy start"""
        self._market_info.market.set_leverage(
            self._market_info.trading_pair, self._leverage
        )
        ...
        if not self.is_algorithm_ready():
            if self._ticks_to_be_ready > 0:
                ...
                if self._ticks_to_be_ready % 10 == 0:
                    self.logger().info(
                        f"📊 Collecting market data... {self._ticks_to_be_ready} ticks remaining"
                    )
            return
        ...
        if not session_positions:
            ...
            if not orders_were_cancelled and self._create_timestamp <= timestamp:
                ...
                if lingering_orders:
                    ...
                    if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                        self.logger().info(
                            f"🔄 Create timestamp expired, forcing cancellation of {len(lingering_orders)} active orders"
                        )
                    ...
                    for order in lingering_orders:
                        ...
                        if order_id is None:
                            self.logger().warning(
                                f"⚠️ Failed to force cancel order without id: {order}"
                            )
                            continue
                        try:
                            self._market_info.market.cancel(
                            ...
                            self.logger().warning(
                                f"⚠️ Failed to force cancel order {order_id}: {e}"
                            )
    ...
    def _collect_market_variables(self, timestamp: float):
        ...
        if self._trading_intensity is None and self._market_info.market.ready:
            self._trading_intensity = TradingIntensityIndicator(
                ...
                price_delegate=OrderBookAssetPriceDelegate(
                    self._market_info.market, self._market_info.trading_pair
                ),
        ...
        if self._trading_intensity:
            ...
            if self._trading_intensity.is_sampling_buffer_full:
                ...
                self._alpha = (
                    Decimal(str(self._alpha)) if self._alpha else Decimal("0.1")
                )
                self._kappa = (
                    Decimal(str(self._kappa)) if self._kappa else Decimal("1.0")
                )
        ...
        try:
            ...
            if hasattr(market, "in_flight_orders"):
                for order_id, in_flight_order in market.in_flight_orders.items():
                    if (
                        in_flight_order.trading_pair == trading_pair
                        and not in_flight_order.is_done
                        and not in_flight_order.is_cancelled
                        and not in_flight_order.is_failure
                    ):
                        active_orders.append(in_flight_order)

                if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                    self.logger().debug(
                        f"📊 Found {len(active_orders)} active in-flight orders for {trading_pair}"
                    )
            ...
            # Method 2: Use limit_orders as fallback
                ...
                limit_orders = [
                    order
                    for order in market.limit_orders
                    if order.trading_pair == trading_pair
                ]
                if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                    self.logger().debug(
                        f"📊 Found {len(limit_orders)} limit orders for {trading_pair}"
                    )
                return limit_orders
            ...
            # Method 3: Fallback to strategy's active_orders
                ...
                if self._logging_options & self.OPTION_LOG_STATUS_REPORT:
                    self.logger().debug(
                        f"📊 Using strategy tracking: {len(strategy_orders)} active orders"
                    )
    ...
    def to_create_orders(self, proposal: Proposal) -> bool:
        ...
        if not (
            ...
            and len(proposal.buys + proposal.sells) > 0
        ):
            return False
        ...
        if strategy_orders > 0:
            if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                self.logger().debug(
                    f"⏳ Strategy tracking shows {strategy_orders} active orders, waiting..."
                )
            return False
        ...
        if exchange_order_count > 0:
            if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                self.logger().info(
                    f"⏳ Exchange shows {exchange_order_count} active orders, waiting for cancellation..."
                )
        ...
        if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
            self.logger().info(
                f"✅ No active orders detected (strategy: {strategy_orders}, exchange: {exchange_order_count}), proceeding with creation"
            )
    ...
    def is_algorithm_ready(self) -> bool:
        ...
        try:
            ...
            return (
                self._avg_vol is not None
                and self._avg_vol.is_sampling_buffer_full
                and self._ticks_to_be_ready <= 0
            )
        except AttributeError:
            # Fallback: derive readiness from volatility buffer only
            buffers_ready = self._avg_vol is not None and getattr(
                self._avg_vol, "is_sampling_buffer_full", False
            )
            if (
                buffers_ready
                and getattr(self._avg_vol, "current_value", None) is not None
            ):
    ...
    def _has_pending_exit_orders(self) -> bool:
        ...
        expired_orders = [
            order_id
            for order_id, timestamp in self._exit_orders.items()
    ...
    def format_status(self) -> str:
        ...
        if positions:
            ...
            for pos in positions:
                ...
                lines.append(
                    f"    {pos.position_side.name}: {pos.amount:.6f} @ {pos.entry_price:.6f} (PnL: {pnl:.4f})"
                )
    ...
    def did_complete_buy_order(self, buy_order_completed_event: BuyOrderCompletedEvent):
        ...
        if self._logging_options & self.OPTION_LOG_MAKER_ORDER_FILLED:
            self.logger().info(
                f"✅ Buy order completed: {buy_order_completed_event.order_id}"
            )

    def did_complete_sell_order(
        self, sell_order_completed_event: SellOrderCompletedEvent
    ):
        """Handle sell order completion"""
        if self._logging_options & self.OPTION_LOG_MAKER_ORDER_FILLED:
            self.logger().info(
                f"✅ Sell order completed: {sell_order_completed_event.order_id}"
            )
    ...
    def did_change_position_mode_succeed(
        ...
        self.logger().info(
            f"✅ Position mode changed to {position_mode_changed_event.position_mode.name}"
        )
    ...
    def did_change_position_mode_fail(
        ...
        self.logger().error(
            f"❌ Failed to change position mode: {position_mode_changed_event}"
        )
