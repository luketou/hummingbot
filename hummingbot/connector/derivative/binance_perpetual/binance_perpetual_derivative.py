...
class BinancePerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _place_order(
        ...
        native_order_type = kwargs.get("binance_order_type")
        ...
        price_match = kwargs.get("price_match")
        reduce_only = kwargs.get("reduce_only", position_action == PositionAction.CLOSE)
        api_params = {"symbol": symbol,
                      "side": "BUY" if trade_type is TradeType.BUY else "SELL",
                      "quantity": amount_str,
                      "type": native_order_type or ("MARKET" if order_type is OrderType.MARKET else "LIMIT"),
                      "newClientOrderId": order_id
                      }
        if order_type.is_limit_type() or native_order_type in {"STOP", "TAKE_PROFIT"}:
        ...
        if stop_price is not None:
            api_params["stopPrice"] = f"{Decimal(stop_price):f}"
        if working_type is not None:
            api_params["workingType"] = working_type
        if price_protect is not None:
            api_params["priceProtect"] = str(price_protect).lower()
        if close_position:
            api_params["closePosition"] = "true"
        if price_match is not None and "price" not in api_params:
            api_params["priceMatch"] = price_match
        if reduce_only and self.position_mode != PositionMode.HEDGE:
