...
class BinancePerpetualDerivativeUnitTest(IsolatedAsyncioWrapperTestCase):
    ...
    def _return_calculation_and_set_done_event(
        self, calculation: Callable, *args, **kwargs
    ):
    ...
    def _get_wrong_symbol_position_risk_api_endpoint_single_position_list(
        self,
    ) -> List[Dict[str, Any]]:
        positions = [
            {
    ...
    def _get_wrong_symbol_account_update_ws_event_single_position_dict(
        self,
    ) -> Dict[str, Any]:
        account_update = {
            "e": "ACCOUNT_UPDATE",
    ...
    @aioresponses()
    async def test_existing_account_position_detected_on_positions_update(
        self, req_mock
    ):
    ...
    async def test_wrong_symbol_position_detected_on_positions_update(self, req_mock):
        ...
        positions = (
            self._get_wrong_symbol_position_risk_api_endpoint_single_position_list()
        )
        req_mock.get(regex_url, body=json.dumps(positions))
    ...
    async def test_set_position_mode_initial_mode_is_none(self, mock_api):
        ...
        url = web_utils.private_rest_url(
            CONSTANTS.CHANGE_POSITION_MODE_URL, domain=self.domain
        )
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        await self.exchange._trading_pair_position_mode_set(
            PositionMode.HEDGE, self.trading_pair
        )
    ...
    @aioresponses()
    async def test_set_position_mode_diff_initial_mode_change_successful(
        self, mock_api
    ):
        self.exchange._position_mode = PositionMode.ONEWAY
        url = web_utils.private_rest_url(
    ...
    async def test_get_collateral_token(self):
        ...
        self.assertEqual(
            margin_asset, self.exchange.get_buy_collateral_token(self.trading_pair)
        )
        self.assertEqual(
            margin_asset, self.exchange.get_sell_collateral_token(self.trading_pair)
        )
    ...
    async def test_buy_order_fill_event_takes_fee_from_update_event(self):
        ...
        self.test_task = self.local_event_loop.create_task(
        ...
        self.assertEqual(
            [TokenAmount(partial_fill["o"]["N"], Decimal(partial_fill["o"]["n"]))],
        ...
        self.assertEqual(Decimal("0"), fill_event.trade_fee.percent)
        self.assertEqual(
            [TokenAmount(complete_fill["o"]["N"], Decimal(complete_fill["o"]["n"]))],
            fill_event.trade_fee.flat_fees,
        )
    ...
    async def test_wrong_symbol_margin_call_event(self):
        ...
        self.assertTrue(
            self._is_logged("INFO", "Margin Required: 0. Negative PnL assets: .")
        )
    ...
    async def test_update_order_fills_from_trades_successful(
        self, req_mock, mock_timestamp
    ):
        self._simulate_trading_rules_initialized()
        ...
        self.assertEqual(
            OrderState.PENDING_CREATE, in_flight_orders["OID1"].current_state
        )
    ...
    async def test_update_order_status_successful(self, req_mock, mock_timestamp):
        ...
        self.assertEqual("8886774", in_flight_orders["OID1"].exchange_order_id)
        self.assertEqual(
            OrderState.PARTIALLY_FILLED, in_flight_orders["OID1"].current_state
        )
        self.assertEqual(1, in_flight_orders["OID1"].leverage)
        self.assertEqual(PositionAction.OPEN, in_flight_orders["OID1"].position)
    ...
    async def test_request_order_status_successful(self, req_mock, mock_timestamp):
        ...
        self.assertEqual(
            order_update.client_order_id, in_flight_orders["OID1"].client_order_id
        )
    ...
    async def test_set_leverage_successful(self, req_mock):
        ...
        success, msg = await self.exchange._set_trading_pair_leverage(
    ...
    async def test_set_leverage_failed(self, req_mock):
        ...
        success, message = await self.exchange._set_trading_pair_leverage(
            trading_pair, leverage
        )
    ...
    async def test_fetch_funding_payment_successful(self, req_mock):
        ...
        regex_url_income_history = re.compile(
            f"^{url}".replace(".", r"\.").replace("?", r"\?")
        )
        ...
        regex_url_funding_info = re.compile(
            f"^{url}".replace(".", r"\.").replace("?", r"\?")
        )
        ...
        self.assertTrue(
            funding_info_logged.trading_pair == f"{self.base_asset}-{self.quote_asset}"
        )

        self.assertEqual(
            funding_info_logged.funding_rate, funding_info["lastFundingRate"]
        )
    ...
    async def test_cancel_order_successful(self, mock_api):
        ...
        canceled_order_id = await self.exchange._execute_cancel(
            trading_pair=self.trading_pair, order_id="OID1"
        )
    ...
    async def test_cancel_order_failed(self, mock_api):
        ...
        await self.exchange._execute_cancel(
            trading_pair=self.trading_pair, order_id="OID1"
        )
    ...
    @aioresponses()
    @patch(
        "hummingbot.connector.derivative.binance_perpetual.binance_perpetual_web_utils.get_current_server_time"
    )
    async def test_place_order_manage_server_overloaded_error_unkown_order(
        ...
        self.exchange._last_poll_timestamp = (
            ...
            - self.exchange.UPDATE_ORDER_STATUS_MIN_INTERVAL
            - 1
        )
        url = web_utils.private_rest_url(CONSTANTS.ORDER_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    ...
    async def test_create_order_min_order_size_failure(self):
        ...
        min_order_size = 3
        mocked_response = self._get_exchange_info_mock_response(
            margin_asset, min_order_size=min_order_size
        )
        trading_rules = await self.exchange._format_trading_rules(mocked_response)
    ...
    async def test_account_info_request_includes_timestamp(
        self, mock_api, mock_seconds_counter
    ):
